...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 to-purple-800 flex items-center justify-center p-4">
      <div className="bg-white bg-opacity-10 backdrop-blur-lg rounded-3xl shadow-2xl p-6 sm:p-10 w-full max-w-sm border border-white border-opacity-20 transform transition-all duration-300 hover:scale-105">
        <h1 className="text-3xl sm:text-4xl font-extrabold text-white text-center mb-6 drop-shadow-lg">
          Countdown Timer
        </h1>

        {isFinished ? (
          <div className="text-red-400 text-6xl font-extrabold text-center my-8 animate-pulse">
            Time's Up!
          </div>
        ) : (
          <div className="text-white text-6xl sm:text-7xl font-mono font-bold text-center mb-8 drop-shadow-lg">
        ...
        <div className="flex justify-center items-center space-x-4 mb-8">
          <input
            ...
            disabled={isRunning}
            className="w-20 p-3 text-center text-white bg-white bg-opacity-20 rounded-xl text-3xl font-mono focus:outline-none focus:ring-2 focus:ring-purple-400 disabled:opacity-50 transition-colors duration-200"
        ...
        <div className="flex justify-center space-x-3 sm:space-x-4">
          {!isRunning && !isFinished && (
            <button
              onClick={startTimer}
              disabled={!canStart}
              className="flex-1 px-4 py-3 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-full shadow-lg transform hover:scale-105 transition-all duration-200 disabled:bg-gray-400 disabled:cursor-not-allowed"
            >
              Start
            </button>
          )}
          {isRunning && (
            <button
              onClick={pauseTimer}
              className="flex-1 px-4 py-3 bg-yellow-500 hover:bg-yellow-600 text-white font-semibold rounded-full shadow-lg transform hover:scale-105 transition-all duration-200"
            >
              Pause
            </button>
          )}
          {showReset && (
            <button
              onClick={resetTimer}
              className="flex-1 px-4 py-3 bg-red-500 hover:bg-red-600 text-white font-semibold rounded-full shadow-lg transform hover:scale-105 transition-all duration-200"
            >
              Reset
            </button>
          )}
      ...
      {/* Audio element for playing alarm. Preload="auto" to ensure it's ready. */}
      <audio ref={audioRef} src={audioSrc} loop={false} preload="auto" />
    </div>
  );
};
